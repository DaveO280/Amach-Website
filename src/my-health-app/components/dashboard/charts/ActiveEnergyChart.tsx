...
const ActiveEnergyChart: React.FC<ActiveEnergyChartProps> = ({ 
  ...
  // Add debugging
  useEffect(() => {
    console.log("ActiveEnergyChart - data received:", data.length);
    if (data.length > 0) {
      console.log("ActiveEnergyChart - first data point:", data[0]);
    }
  }, [data]);
  ...
  const chartData = useMemo(() => {
    ...
    console.log("ActiveEnergyChart - processed chart data:", result.length);
    if (result.length > 0) {
      console.log("ActiveEnergyChart - first chart data point:", result[0]);
    }
      ...
      <div style={{ width: '100%', height }}>
        ...
        {chartData.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <p className="text-gray-500">No active energy data available for the selected time period.</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={height}>
            ...
            >
              ...
              />
              <Tooltip 
                formatter={(value: number) => [`${value} ${unit}`, 'Active Energy']}
