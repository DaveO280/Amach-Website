...
/**
 * Process daily active energy data for charting.
 * Groups by day, sums total energy, and returns sorted array.
 */
export const processDailyActiveEnergyData = (
  data: HealthData[],
): Array<{ day: string; date: Date; energy: number; count: number }> => {
...
/**
 * Process daily exercise time data for charting.
 * Groups by day, sums total time (converts seconds to minutes if needed), and returns sorted array.
 */
export const processDailyExerciseTimeData = (
  data: HealthData[],
): Array<{ day: string; date: Date; time: number; count: number }> => {
  const dailyData: Record<
    string,
  ...
  data.forEach((point) => {
    try {
      ...
      if (!isNaN(value)) {
        if (!dailyData[dayKey]) {
          dailyData[dayKey] = { values: [], total: 0, unit: point.unit || "" };
        }
        dailyData[dayKey].values.push(value);
        dailyData[dayKey].total += value;
        if (point.unit) {
