...
const ExerciseTimeChart: React.FC<ExerciseTimeChartProps> = ({ 
  ...
  // Add debugging
  useEffect(() => {
    console.log("ExerciseTimeChart - data received:", data.length);
    if (data.length > 0) {
      console.log("ExerciseTimeChart - first data point:", data[0]);
    }
  }, [data]);
  ...
  const chartData = useMemo(() => {
    ...
    const dailyData: Record<string, { values: number[], total: number, unit: string }> = {};
    ...
    console.log("ExerciseTimeChart - processed chart data:", result.length);
    if (result.length > 0) {
      console.log("ExerciseTimeChart - first chart data point:", result[0]);
    }
  ...
  const unit = data.length > 0 && data[0].unit ? 
    (data[0].unit.toLowerCase().includes('sec') ? 'min' : data[0].unit) : 
    'min';
      ...
      <div style={{ width: '100%', height }}>
        ...
        {chartData.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <p className="text-gray-500">No exercise time data available for the selected time period.</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={height}>
            ...
            >
              ...
              />
              <Tooltip 
                formatter={(value: number) => [`${value} ${unit}`, 'Exercise Time']}
