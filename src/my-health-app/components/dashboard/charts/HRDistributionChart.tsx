...
const zoneDescriptions = {
  zone1: { name: 'Very Light', color: '#22c55e', description: 'Very light activity, warm up' },
  zone2: { name: 'Light', color: '#84cc16', description: 'Basic endurance training' },
  zone3: { name: 'Moderate', color: '#eab308', description: 'Aerobic endurance training' },
  zone4: { name: 'Hard', color: '#f97316', description: 'Anaerobic threshold training' },
  zone5: { name: 'Maximum', color: '#ef4444', description: 'Maximum performance, sprint' }
};
...
const HRDistributionChart: React.FC<HRDistributionChartProps> = ({ 
  ...
  const zones = useMemo(() => calculateHRZones(calculatedMaxHR), [calculatedMaxHR]);
  ...
  const zoneData = useMemo(() => {
    ...
    return Object.entries(zoneCounts).map(([zone, count]) => {
      ...
      return {
        ...
        description: zoneDescriptions[zone as keyof typeof zoneDescriptions].description,
  ...
  const hrDistribution = useMemo(() => {
    ...
    data.forEach(point => {
      try {
        ...
        console.error("Error processing heart rate distribution data point:", e);
      }
    });
    ...
    return Object.entries(buckets)
      .map(([bucket, count]) => ({
        ...
        count,
        zone: Object.entries(zones).find(([_, range]) => 
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white p-2 border border-gray-200 rounded shadow-sm">
          ...
          <p>{data.range}</p>
          <p className="font-medium">{data.percentage}% ({data.count} readings)</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div className="w-full">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <h4 className="text-lg font-medium mb-2 text-center">Heart Rate Zone Distribution</h4>
        ...
        <div>
          <h4 className="text-lg font-medium mb-2 text-center">Heart Rate Distribution</h4>
          <div style={{ width: '100%', height }}>
            <ResponsiveContainer>
              <BarChart
                data={hrDistribution}
              ...
              >
                ...
                <XAxis 
                  dataKey="bucket" 
                  label={{ value: 'BPM Range', position: 'insideBottom', offset: -5 }}
                />
                <YAxis 
                ...
                <Bar dataKey="count">
                  {hrDistribution.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={entry.zone !== 'none' 
                        ? zoneDescriptions[entry.zone as keyof typeof zoneDescriptions].color 
                        : '#94a3b8'
                      } 
                    />
                  ))}
      ...
      <div className="mt-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-2">
          {Object.entries(zoneDescriptions).map(([zone, info]) => (
            ...
            >
              <div className="text-sm font-medium" style={{ color: info.color }}>{info.name}</div>
              <div className="text-xs text-gray-500">{info.description}</div>
              <div className="text-sm mt-1">
                {zones[zone as keyof typeof zones].min}-{zones[zone as keyof typeof zones].max} bpm
              </div>
            </div>
          ))}
        </div>
