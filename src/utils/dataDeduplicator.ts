...
export const isWatchData = (record: HealthDataPoint): boolean => {
  ...
  // Check device field first - this is more reliable
  if (deviceInfo.toLowerCase().includes("watch")) {
    return true;
  }

  // Check if device contains iPhone - this indicates phone data
  if (deviceInfo.toLowerCase().includes("iphone")) {
    return false;
  }

  // Fallback to source field check
  if (sourceInfo.toLowerCase().includes("watch")) {
    return true;
  }
...
export const deduplicateCumulativeData = (
  ...
  // Debug logging for step count
  if (metricId === "HKQuantityTypeIdentifierStepCount") {
...
/**
 * Debug function to analyze device detection for step count data
 */
export const debugStepCountDeviceDetection = (): void => {
  // Removed all logging statements for cleaner output
};
