...
const HeartRateChart: React.FC<HeartRateChartProps> = ({ 
  ...
  // Add debugging
  useEffect(() => {
    console.log("HeartRateChart - data received:", data.length);
    if (data.length > 0) {
      console.log("HeartRateChart - first data point:", data[0]);
    }
  }, [data]);
  ...
  const chartData = useMemo(() => {
    ...
    const dailyData: Record<string, { values: number[], min: number, max: number }> = {};
    ...
    console.log("HeartRateChart - processed chart data:", result.length);
    if (result.length > 0) {
      console.log("HeartRateChart - first chart data point:", result[0]);
    }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 border border-gray-200 rounded shadow-sm">
          ...
          <p className="text-gray-500 text-sm mt-1">Readings: {payload[0].payload.count}</p>
        </div>
      );
    }
    return null;
      ...
      <div style={{ width: '100%', height }}>
        ...
        {chartData.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <p className="text-gray-500">No heart rate data available for the selected time period.</p>
          </div>
        ) : (
